class Student
  ...
  def self.new_from_db(row)
    ...
    new_student = self.new.tap do |student|
      student.id = row[0]
      student.name = row[1]
      student.grade = row[2]
    end
  ...
  def self.all
    ...
    SQL
    DB[:conn].execute(sql).each do |student_row|
      all << self.new_from_db(student_row)
    end
    all
  end
  ...
  def self.all_students_in_grade_x(grade)
    sql = <<-SQL
      ...
      WHERE grade = ?;
    SQL
    DB[:conn].execute(sql, grade).each do |student_row|
      self.new_from_db(student_row)
    end
  end
end
