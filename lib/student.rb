...
class Student
  ...
  def self.count_all_students_in_grade_9
    sql = <<-SQL
    SELECT COUNT (id) FROM students WHERE students.grade = 9
    SQL
  ...
  def self.students_below_12th_grade
    sql = <<-SQL
    SELECT COUNT (id) FROM students WHERE students.grade < 12
    SQL
  ...
  def self.first_student_in_grade_10
    sql = <<-SQL
    SELECT MIN(students.id) FROM students WHERE students.grade = 10
    SQL
